...
var Sara = (function () {
    ...
    return {
        init: function () {
            console.log('Init');
            var paralaxes = $('.paralax'),
                $paralax,
                paralaxSrc;

            for (var i = 0, len = paralaxes.length; i < len; i++) {
                $paralax = $(paralaxes[i]);
                paralaxSrc = $paralax.attr('data-src');

                $paralax.css('background-image', 'url(' + paralaxSrc + ')');
                console.log('Paralax-src: ', paralaxSrc);
            }
        },

        paralaxMode: function () {
